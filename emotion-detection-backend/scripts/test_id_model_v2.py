"""
Test Indonesian Model v2 with comprehensive test suite.
Uses same preprocessor as training.
"""
import os
import sys
import pickle
...
MODEL_DIR = Path(__file__).parent.parent / "app" / "ml" / "saved_models_id_v2"

def load_model():
    model = tf.keras.models.load_model(str(MODEL_DIR / "emotion_lstm_id_v2.h5"))
...
def main():
    print("="*70)
    print("🇮🇩 TESTING INDONESIAN MODEL V2 (with Sastrawi Preprocessing)")
    print("="*70)
    
    model, tokenizer, label_encoder = load_model()
    preprocessor = IndonesianTextPreprocessor(remove_stopwords=False, use_stemming=True)
    ...
    for expected, texts in test_cases.items():
        ...
        for text in texts:
            emotion, conf = predict(text, model, tokenizer, label_encoder, preprocessor)
